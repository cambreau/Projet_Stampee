...
export const badgeFavCoupCoeur = async (enchere, parent) => {
  ...
  if (enchere["coupCoeurLord"] === 1) {
    ajoutBadgeCCL(divBadge);
  }
  ajoutBadgeFav(divBadge, enchere);
};

/**
 * Ajout badge CCL
 * @param HTMLElement parentBadge
 */
const ajoutBadgeCCL = (parentBadge) => {
  const badgeCCL = document.createElement("pciture");
  badgeCCL.classList.add("icon_grand");
  parentBadge.appendChild(badgeCCL);
